...
| Renderer / cast selection | LibVLC renderer bridge | MobileVLCKit renderer discovery and switching | Not yet | Not yet |
| Network share discovery and browsing | LibVLC discovery and browser | MobileVLCKit LAN discovery and folder parsing | Not yet | Not yet |
...
iOS Picture in Picture remains deliberately unavailable with the public MobileVLCKit SPM binary.
Upstream VLC iOS uses its private `VLCPictureInPictureDrawable` protocol and controller, neither of
which is exported by that binary; AVPlayer PiP cannot render MobileVLCKit's drawable. The shared
control therefore stays hidden rather than advertising a no-op until the decoder package exposes a
supported PiP output path.
