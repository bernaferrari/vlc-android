...
class WidgetRepository(private val widgetDao: WidgetDao) {
    ...
    suspend fun createNew(context: Context, appWidgetId: Int): Widget {
        val widget = Widget(appWidgetId, 0, 0, 0, 0, true, ContextCompat.getColor(context, R.color.reimagined_canvas_light), ContextCompat.getColor(context, R.color.reimagined_text_light), 10, 10, 100, showConfigure = true, showSeek = true, showCover = true)
        addWidget(widget)
        return widget
    }
