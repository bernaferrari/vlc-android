...
class PreferencesAdvanced : BasePreferenceFragment(), SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    private fun onConfirmDelete(reason: Int) {
        when (reason) {
        ...
        when (preference.key) {
            ...
            "clear_history" -> {
                requireActivity().showConfirmDeleteComposeDialog(
                    ...
                    onConfirmDelete(RESULT_VALUE_CLEAR_HISTORY)
                }
                return true
            }
            "clear_media_db" -> {
                ...
                if (medialibrary.isWorking) {
                    ...
                    requireActivity().showConfirmDeleteComposeDialog(
                        ...
                        onConfirmDelete(RESULT_VALUE_CLEAR_MEDIA_DATABASE)
                    }
                    return true
                }
            }
            "clear_app_data" -> {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    requireActivity().showConfirmDeleteComposeDialog(
                        ...
                        onConfirmDelete(RESULT_VALUE_CLEAR_APP_DATA)
                    }
                } else {
