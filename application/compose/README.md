...
- **Pointers to host files** (update this list as new hosts land):
  ...
  - Audio player chrome hosts: `application/vlc-android/src/org/videolan/vlc/gui/audio/AudioPlayer.kt` + `audio_player.xml` / `layout-land/audio_player.xml` (`VLCAudioPlayerChips`, `VLCAudioQueueProgressPill`, `VLCAudioHeaderBackground`, `VLCAudioHeaderDivider`, `VLCAudioHeaderTimeLabel`, `VLCAudioHeaderActionButton`, `VLCAudioHeaderPlayPauseButton`, `VLCAudioHeaderTransportButton`, `VLCAudioSeekHudButton`, `VLCAudioSeekDelayLabel`, `VLCAudioAbRepeatMarker`, `VLCAudioTrackInfoText`, `VLCAudioTimelineTimeLabel`; compose-q9r.3 / compose-68e; includes foldable hinge affordances, A-B repeat markers, landscape track info text, and full-player timeline labels)
