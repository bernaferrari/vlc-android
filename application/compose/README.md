...
- Compose-hosted root screens with remaining subpage follow-up:
  - `PreferencesActivity` root settings list plus Interface, Video, Subtitles, Audio, Casting, Parental Control, Remote Access, and Android Auto subpages (former phone root/Interface/Video/Subtitles/Audio/Casting/Parental/Remote Access/Android Auto preference Fragments removed; larger `BasePreferenceFragment` subpages still tracked separately)
...
- **Pointers to host files** (update this list as new hosts land):
  ...
  - Preferences root/subpage host: `application/vlc-android/src/org/videolan/vlc/gui/preferences/PreferencesActivity.kt` + `PreferencesRootScreen.kt` + `PreferencesComposeSubpages.kt` (bd compose-qeh / compose-0ep; removes the phone root, Interface, Video, Subtitles, Audio, Casting, Parental Control, Remote Access, and Android Auto preference Fragments while preserving the remaining larger preference fragments as follow-up work)
